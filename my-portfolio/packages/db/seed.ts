...
import { PROJECTS, EXPERIENCES, EVENTS } from '../../apps/portfolio/src/data/registry';
...
function main() {
  ...
  EXPERIENCES.forEach(e => e.technologies?.forEach(t => skillSet.add(t)));
  EVENTS.forEach(ev => ev.technologies?.forEach(t => skillSet.add(t)));

  const skillList = Array.from(skillSet).sort();
  const skillMap = new Map<string, number>();

  sqlLines.push('-- Insert Skills');
  skillList.forEach((skillName, index) => {
    const id = index + 1;
  ...
  PROJECTS.forEach((proj, index) => {
    ...
    proj.technologies?.forEach(techName => {
  ...
  EVENTS.forEach(evt => {
    ...
    // Skills
    evt.technologies?.forEach(techName => {
      const skillId = skillMap.get(techName);
      if (skillId) {
        sqlLines.push(
          `INSERT INTO timeline_skills (timeline_id, skill_id) VALUES (${timelineId}, ${skillId});`
