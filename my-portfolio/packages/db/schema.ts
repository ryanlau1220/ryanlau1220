...
export const timelineCategoryEnum = [
  'education',
  'internship',
  'hackathon',
  'other'
...
export const timelineAchievements = sqliteTable('timeline_achievements', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  timelineId: integer('timeline_id').notNull().references(() => timeline.id, { onDelete: 'cascade' }),
  content: text('content').notNull()
});

// Timeline-Skill Junction Table
export const timelineSkills = sqliteTable('timeline_skills', {
  timelineId: integer('timeline_id').notNull().references(() => timeline.id, { onDelete: 'cascade' }),
  skillId: integer('skill_id').notNull().references(() => skills.id, { onDelete: 'cascade' })
}, (t) => ({
